...
export function buildGenerativeUiProviderPrompt(
  ...
  return [
    ...
    "- StudyDeck(title: string, summary: string, cards: Flashcard[], quiz: Quiz[])",
    "",
    "Design guidance:",
    "- Prefer HeroSummary, Badge, Callout, CodeBlock, and tone/icon arguments for study apps and reference widgets.",
    ...
    "- Flashcards render as equal neutral cards; do not assign a different tone to every flashcard.",
    "",
    "Example:",
    "```openui",
    'root = Widget("Study cards", [status, hero, tip, code, cards])',
    'status = SourceStatus("Sources", ["loaded: notes.pdf"])',
    'm1 = Metric("Cards", "2", "violet", "brain")',
    'hero = HeroSummary("Python review", "Review the key ideas, then test recall.", [m1], "violet", "study")',
    'tip = Callout("Exam tip", "Use join() instead of repeated string concatenation in loops.", "amber", "idea")',
    'code = CodeBlock("python", "result = \'\'.join(chunks)")',
    "cards = Flashcards([card1, card2])",
    'card1 = Flashcard("Why use join()?", "It allocates once and avoids O(n^2) concatenation behavior.", "cyan", "code")',
    'card2 = Flashcard("Why use casefold()?", "It is safer for international caseless matching.", "emerald", "check")',
    "```",
    "",
    groundingSection,
    "User request:",
