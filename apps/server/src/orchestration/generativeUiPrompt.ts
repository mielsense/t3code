...
export function buildGenerativeUiProviderPrompt(
  ...
  return [
    ...
    '- Text(text: string, variant?: "body" | "muted" | "heading" | "caption")',
    "- List(items: string[])",
    "- Table(headers: string[], rows: string[][])",
    "- Metric(label: string, value: string)",
    "- Button(label: string)",
    "- Tabs(items: Tab[])",
    "- Tab(title: string, children: Component[])",
    ...
    "- HeroSummary(title: string, subtitle: string, metrics: Metric[])",
    "- StatGrid(metrics: Metric[])",
    "- Timeline(items: TimelineItem[])",
    "- TimelineItem(title: string, subtitle: string, detail: string)",
    ...
    'intro = Text("Review the key ideas, then test recall.", "muted")',
    "cards = Flashcards([card1, card2])",
    'card1 = Flashcard("What is photosynthesis?", "The process plants use to convert light into chemical energy.")',
    'card2 = Flashcard("Where does it happen?", "Inside chloroplasts.")',
    "```",
    "",
    groundingSection,
    "User request:",
